#!/bin/bash
# sbatch preparation script for 3dunet 1.8.2
...
if [ 3dunet_type == "train" ]; then
    ...
    ## exist checks before the sbatch command
    # -var 3dunet type (should)
    # -d output dir (should not)
    # -f config yaml (should)
    # -var input session (train: should not, predict: should)
    # -d input dir (train: should not, predict: should)
    # -f sbatch script (should)
